...
* Generated by SAP_LOCAL_DOWNPORT_ASSISTANT Version 5.40 on 08.01.2016 with option ALL

* The individual coding starts at 'FORM update_01.' ( line 3.648 )
...
CONSTANTS: activation_mode VALUE '2',      "1 = one step, 2 = two step
           ...
           'AOBJ''CHDO''IWPR''IWMO''IWSV''SUCD''SUSC''SUSK''DIOB''DTRF''DROU''DTRC''BSVO''BSVI''BSVV''SCP2''TOBJ''PCLA''...''DTMT''DTMU''DTMR''DTMS''PSCS''PMKS''PFCS''POCS''PCYS''...''LODS''LODE''LOIE''LODC''SHI3''SHI5''SHI6''ECTD'.
...
DATA: BEGIN OF gc_objtext,
        ...
        "ENHO
        notfound          TYPE ddtext VALUE 'Not found / Unknown', "#EC NOTEXT
...
*  info2    = '- If an error occurs repeat the same step again until the error is solved.'. "#EC NOTEXT "1900
*  info3    = '- It does not harm if a step is executed several times.'. "#EC NOTEXT  "1900
*  info4    = '- Nearlly all popups can be closed by pressing ENTER.'. "#EC NOTEXT  "1900

  testrun1 = 'Step 1'. "#EC NOTEXT                                            "1900
  testrun2 = 'Test Run'.                         "#EC NOTEXT      "1900
*  testrun2 = 'TESTRUN: Check prerequisites and show the list of objects without updating them.'. "#EC NOTEXT      "1900
  update1  = 'Step 2'. "#EC NOTEXT                                            "1900
  update2  = 'Update & Activate'.                    "#EC NOTEXT  "1900
*  update2  = 'UPDATE & ACTIVATE:  Update the objects if needed and perform activation.'. "#EC NOTEXT  "1900
*  update3  = 'Hint: For activation chose ''online'' mode unless this fails with time out.'. "#EC NOTEXT  "1900
  genview1 = 'Step 3'. "#EC NOTEXT                                            "1900
  genview2 = 'Generate Maintenance Dialogs'.        "#EC NOTEXT   "1900
*  genview2 = 'GENVIEW: Generate Maintenance Dialogs. Perform this step after successful UPDATE.'. "#EC NOTEXT   "1900
  showlog1 = 'Info'. "#EC NOTEXT                                            "1900
  showlog2 = 'Show Logs'. "#EC NOTEXT                                            "1900
*  showlog2 = 'SHOWLOGS: Display logs of previous runs.'. "#EC NOTEXT                                            "1900
...
FORM prepare_table USING i_tabname    TYPE tabname
  ...
  SELECT * FROM (i_tabname) INTO TABLE <lt_entries> WHERE (lt_where) ORDER BY PRIMARY KEY. "#EC CI_DYNTAB
...
*    IF NOT is_dd08v IS INITIAL OR it_dd05m IS SUPPLIED.
...
*               UPDATE tadir SET masterlang = space "clear language and let SCWB/SNOTE set the correct language
                DELETE FROM tadir
                           WHERE pgmid      EQ ls_tadir_key-pgmid
                             AND object     EQ ls_tadir_key-object
                             AND obj_name   EQ ls_tadir_key-obj_name.
...
*    DATA: lr_siw_resource_access TYPE REF TO cl_siw_resource_access.                 ">=SAPBASIS700
*    CALL METHOD cl_siw_resource_access=>s_get_instance                               ">=SAPBASIS700
*      RECEIVING                                                                      ">=SAPBASIS700
*        r_access = lr_siw_resource_access.                                           ">=SAPBASIS700
*                                                                                     ">=SAPBASIS700
*    TRY.                                                                             ">=SAPBASIS700
*        CALL METHOD lr_siw_resource_access->if_siw_repository_writer~write_textpool  ">=SAPBASIS700
*          EXPORTING                                                                  ">=SAPBASIS700
*            i_tab_textpool = lt_textpool                                             ">=SAPBASIS700
*            i_prog         = i_progname                                              ">=SAPBASIS700
*            i_langu        = i_langu.                                                ">=SAPBASIS700
*      CATCH cx_siw_resource_failure .                                                ">=SAPBASIS700
*    ENDTRY.                                                                          ">=SAPBASIS700

*   CALL FUNCTION 'RV_REPORT_WRITE_LANGU'  "not in SAP_BASIS
*   PERFORM INSERT_TEXTPOOL in PROGRAM saplSWU5 TABLES lt_textpool
*                    USING  i_progname
*                           i_LANGU.
...
FORM update_01. " TABD NORG_BPOM_BUFFER ( DE , source: EWI )
  ...
  CALL METHOD lcl_wb=>create_table     "#EC *
    EXPORTING
      i_tabname    = 'NORG_BPOM_BUFFER'
      i_devclass   = 'N1BASE_MED'
      i_exclass    = '1'
      i_authclass  = '00'
      i_tabclass   = 'TRANSP'
      i_contflag   = 'L'
      ...
      i_roworcolst = 'C'
      i_langu      = 'D'
      i_ddtext     = 'IS-H MED: Org.Einheit Einrichtung Gültigkeit Puffer für BPOM'.
...
FORM update_02. " TABT NORG_BPOM_BUFFER ( DE , source: EWI )
...
FORM update_03. " TTYD ISHMED_T_NORG_BPOM_BUFFER ( DE , source: EWI )
  ...
  CALL METHOD lcl_wb=>create_table_type     "#EC *
    EXPORTING
      i_typename   = 'ISHMED_T_NORG_BPOM_BUFFER'
      i_devclass   = 'N1BASE_MED'
      i_langu      = 'D'
      i_ddtext     = 'Typ für Tabelle NORG_BPOM_BUFFER'
      i_rowtype    = 'NORG_BPOM_BUFFER'
      i_accessmode = 'T'
      i_keydef     = 'D'
      i_keykind    = 'N'
      it_dd43v     = lt_dd43v
...
FORM update_04. " DOCU COCL_ISHMED_MSCHED_PG           GET_DEFAULT_HEADLINES ( DE , source: EWI )
  CALL METHOD lcl_wb=>create_other_docu     "#EC *
    EXPORTING
      i_devclass  = 'N1APMG_MED'
      i_docname   = 'COCL_ISHMED_MSCHED_PG           GET_DEFAULT_HEADLINES'
      i_langu     = 'D'
      i_line      =:'U1 &FUNCTIONALITY&',                                                                          "#EC NOTEXT
                    'AS Mit dieser Methode erhalten Sie die standardmäßigen Beschriftungen der',                   "#EC NOTEXT
                    '   Registerkarten, der Spalten und der Resourcen.',                                           "#EC NOTEXT
...
FORM update_05. " REPT RN1_FILL_NORG_BPOM_BUFFER ( DE , source: EWI )
  CALL METHOD lcl_wb=>create_program_text    "#EC *
    EXPORTING
      i_progname  = 'RN1_FILL_NORG_BPOM_BUFFER'
      i_devclass  = 'N1BASE_MED'
      i_langu     = 'D':
      i_id        = 'I'
      i_key       = '001'
      i_text      = 'Diesen Report ist gültig nur für Systeme mit SAP OM aktiv!'  "#EC NOTEXT
      i_length    = '114',

      i_id        = 'I'
      i_key       = '002'
      i_text      = 'Keine Zeilen wurden von der Datenbank gelöscht!'  "#EC NOTEXT
      i_length    = '82',

      i_id        = 'I'
      i_key       = '003'
      i_text      = ' Zeilen wurden von der Datenbank gelöscht!'  "#EC NOTEXT
      i_length    = '78',

      i_id        = 'I'
      i_key       = '004'
      i_text      = 'Keine Daten können von der SAP OM Struktur übernommen werden!'  "#EC NOTEXT
      i_length    = '94',

      i_id        = 'I'
      i_key       = '005'
      i_text      = ' Einträge die von der SAP OM Struktur übernommen wurden!'  "#EC NOTEXT
      i_length    = '82',

      i_id        = 'I'
      i_key       = '006'
      i_text      = 'Fehler bei Einfügen der Daten!'  "#EC NOTEXT
      i_length    = '52',

      i_id        = 'I'
      i_key       = '007'
      i_text      = 'Keine Einträge wurden aktualisiert. Testmodus ist eingeschaltet.'  "#EC NOTEXT
      i_length    = '128',

      i_id        = 'I'
      i_key       = 'B01'
      i_text      = 'Verarbeitungsmodus:'  "#EC NOTEXT
      i_length    = '29',

      i_id        = 'R'
      i_key       = ''
      i_text      = 'Program für Einfügen der Daten in Tabelle NORG_BPOM_BUFFER'  "#EC NOTEXT
      i_length    = '58',

      i_id        = 'S'
      i_key       = 'P_DELETE'
      i_text      = 'Nur Löschen (ohne Einfügen)'  "#EC NOTEXT
      i_length    = '35',
...
FORM update.
  ...
  APPEND 'NORG_BPOM_BUFFER'                       TO gt_tabname.   "#EC NOTEXT
  APPEND 'ISHMED_T_NORG_BPOM_BUFFER'              TO gt_ttypname.  "#EC NOTEXT

  PERFORM update_01. " TABD NORG_BPOM_BUFFER ( DE , source: EWI )
  PERFORM update_02. " TABT NORG_BPOM_BUFFER ( DE , source: EWI )
  PERFORM update_03. " TTYD ISHMED_T_NORG_BPOM_BUFFER ( DE , source: EWI )
  PERFORM update_04. " DOCU COCL_ISHMED_MSCHED_PG           GET_DEFAULT_HEADLINES ( DE , source: EWI )
  PERFORM update_05. " REPT RN1_FILL_NORG_BPOM_BUFFER ( DE , source: EWI )
