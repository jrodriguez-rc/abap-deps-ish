class CL_ISH_PRC_PATIENT definition
  public
  create protected .

*"* public components of class CL_ISH_PRC_PATIENT
...
public section.
  ...
  constants CO_OTYPE_PRC_PATIENT type ISH_OBJECT_TYPE value 6007. "#EC NOTEXT
  ...
  class-methods CREATE
    exporting
      ...
      value(ER_INSTANCE) type ref to CL_ISH_PRC_PATIENT
    changing
...
protected section.
*"* protected components of class CL_ISH_PRC_PATIENT
*"* do not include other source files here!!!
  ...
  data GR_COMP_PATIENT type ref to CL_ISH_COMP_PATIENT .
  data GR_PRC_PATIENT type ref to CL_ISH_PRC_PATIENT .
  class-data G_COMP_PATIENT_CLASSNAME type STRING .
  ...
  methods COMPLETE_CONSTRUCTION
    ...
    changing
      ...
      !CR_ERRORHANDLER type ref to CL_ISHMED_ERRORHANDLING optional .
private section.
*"* private components of class CL_ISH_PRC_PATIENT
*"* do not include other source files here!!!
ENDCLASS.



CLASS CL_ISH_PRC_PATIENT IMPLEMENTATION.
...
* Handle process environment.
  ...
  IF gr_environment IS INITIAL.
    CALL METHOD cl_ish_fac_environment=>create
      EXPORTING
        i_program_name = 'CL_ISH_PRC_PATIENT'
      IMPORTING
...
* handle comp order
  CALL METHOD cl_ish_comp_patient=>create
    EXPORTING
      i_classname           = g_comp_patient_classname
...
*    I_CALLER              = SY-REPID
  ...
  IF NOT lr_instance IS INITIAL.
    IF lr_instance->is_inherited_from(
              cl_ish_comp_patient=>co_otype_comp_patient ) = on.
      gr_comp_patient ?= lr_instance.
    ENDIF.
  ENDIF.
  IF gr_comp_patient IS INITIAL.
    e_rc = 1.
    EXIT.
  ENDIF.

* get defined screens from comp order
  CALL METHOD gr_comp_patient->get_defined_screens
    RECEIVING
  ...
  IF sy-subrc = 0 AND
    ...
    IF lr_screen->is_inherited_from(
      ...
      gr_scr_patient ?= lr_screen.
    ENDIF.
  ENDIF.
  IF gr_scr_patient IS INITIAL.
    e_rc = 1.
  ENDIF.
...
* Handle component classnames.
  g_comp_patient_classname = 'CL_ISH_COMP_PATIENT'.
...
* screen class
  CALL METHOD gr_scr_patient->set_instance_for_display.

* dynpro + program
  CALL METHOD gr_scr_patient->get_parent
    IMPORTING
      es_parent = gs_parent.
  es_parent = gs_parent.
...
METHOD CREATE .
  ...
  DATA: lr_prc_patient
        TYPE REF TO cl_ish_prc_patient.

* Create the object.
  CREATE OBJECT lr_prc_patient.

* Complete construction.
  CALL METHOD lr_prc_patient->complete_construction
    IMPORTING
      e_rc            = e_rc
    CHANGING
...
METHOD IF_ISH_IDENTIFY_OBJECT~GET_TYPE .
  ...
  e_object_type = co_otype_prc_patient.
...
METHOD IF_ISH_IDENTIFY_OBJECT~IS_INHERITED_FROM .
  ...
  IF i_object_type = co_otype_prc_patient.
