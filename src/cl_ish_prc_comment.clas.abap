class CL_ISH_PRC_COMMENT definition
  public
  create protected .

*"* public components of class CL_ISH_PRC_COMMENT
...
public section.
  ...
  constants CO_OTYPE_PRC_COMMENT type ISH_OBJECT_TYPE value 6019. "#EC NOTEXT
  ...
  class-methods CREATE
    exporting
      ...
      value(ER_INSTANCE) type ref to CL_ISH_PRC_COMMENT
    changing
...
protected section.
*"* protected components of class CL_ISH_PRC_COMMENT
*"* do not include other source files here!!!
  ...
  data GR_COMP_COMMENT type ref to CL_ISH_COMP_COMMENT .
  data GR_PRC_COMMENT type ref to CL_ISH_PRC_COMMENT .
  class-data G_COMP_COMMENT_CLASSNAME type STRING .
  ...
  methods COMPLETE_CONSTRUCTION
    ...
    changing
      ...
      !CR_ERRORHANDLER type ref to CL_ISHMED_ERRORHANDLING optional .
private section.
*"* private components of class CL_ISH_PRC_COMMENT
*"* do not include other source files here!!!
ENDCLASS.



CLASS CL_ISH_PRC_COMMENT IMPLEMENTATION.
...
* Handle process environment.
  ...
  IF gr_environment IS INITIAL.
    CALL METHOD cl_ish_fac_environment=>create
      EXPORTING
        i_program_name = 'CL_ISH_PRC_COMMENT'
      IMPORTING
...
* handle comp order
  CALL METHOD cl_ish_comp_comment=>create
    EXPORTING
      i_classname          = g_comp_comment_classname
  ...
  IF NOT lr_instance IS INITIAL.
    IF lr_instance->is_inherited_from(
              cl_ish_comp_comment=>co_otype_comp_comment ) = on.
      gr_comp_comment ?= lr_instance.
    ENDIF.
  ENDIF.
  IF gr_comp_comment IS INITIAL.
    e_rc = 1.
    EXIT.
  ENDIF.

* get defined screens from comp order
  CALL METHOD gr_comp_comment->get_defined_screens
    RECEIVING
  ...
  IF sy-subrc = 0 AND
    ...
    IF lr_screen->is_inherited_from(
              cl_ish_scr_comment=>co_otype_scr_comment ) = on.
      gr_scr_comment ?= lr_screen.
    ENDIF.
  ENDIF.
  IF gr_scr_comment IS INITIAL.
    e_rc = 1.
  ENDIF.

* initialize the screen
  CALL METHOD gr_scr_comment->initialize
    EXPORTING
...
* Handle component classnames.
  g_comp_comment_classname = 'CL_ISH_COMP_COMMENT'.
...
* screen class
  CALL METHOD gr_scr_comment->set_instance_for_display.

* dynpro + program
  CALL METHOD gr_scr_comment->get_parent
    IMPORTING
      es_parent = gs_parent.
  es_parent = gs_parent.
...
METHOD create .
  ...
  DATA: lr_prc_comment
        TYPE REF TO cl_ish_prc_comment.

* Create the object.
  CREATE OBJECT lr_prc_comment.

* Complete construction.
  CALL METHOD lr_prc_comment->complete_construction
    IMPORTING
      e_rc            = e_rc
    CHANGING
...
METHOD IF_ISH_IDENTIFY_OBJECT~GET_TYPE .
  ...
  e_object_type = co_otype_prc_comment.
...
METHOD IF_ISH_IDENTIFY_OBJECT~IS_INHERITED_FROM .
  ...
  IF i_object_type = co_otype_prc_comment.
...
METHOD OK_CODE_SCREEN .
  ...
  CALL METHOD gr_scr_comment->ok_code_screen
    IMPORTING
      e_rc           = e_rc
    CHANGING
      c_errorhandler = cr_errorhandler
