...
* Generated by SAP_LOCAL_DOWNPORT_ASSISTANT Version 8.04 on 04.10.2021 with option ALL

* The individual coding starts at 'FORM update_01.' ( line 3.897 )
...
DATA:      myrepid         TYPE progname VALUE 'SAP_LOCAL_DOWNPORT_ASSISTANT'.  "must not be constant because of external performs
...
DATA: approved_logical_objects TYPE string. "filled in method lcl_wb=>init
...
DATA: BEGIN OF gc_objtext,
        ...
        maint_obj_table   TYPE ddtext VALUE 'Maintenance Object (Table)', "#EC NOTEXT
        maint_obj_view    TYPE ddtext VALUE 'Maintenance Object (View)', "#EC NOTEXT
        maint_obj_cluster TYPE ddtext VALUE 'Maintenance Object (View Cluster)', "#EC NOTEXT
        maint_obj_variant TYPE ddtext VALUE 'Maintenance Object (View Variant)', "#EC NOTEXT
        "METH
        ...
        maint_view_func   TYPE ddtext VALUE 'Function Module (Maintenance View)', "#EC NOTEXT
        "FUGR
...
CONSTANTS: c_bal_object     TYPE balobj_d  VALUE 'SNOTE',
           ...
           c_lf             TYPE char1      VALUE cl_abap_char_utilities=>newline,
           c_htab           TYPE char1      VALUE cl_abap_char_utilities=>horizontal_tab,
           c_hex00          TYPE x VALUE 0,
...
DATA: gc_sapbasis_infinity TYPE sysaprl VALUE '757'.  "SAP_BASIS which is used in infinity systems, e.g. ER9
...
CONSTANTS: BEGIN OF gc_dok,   "see also include SDOC_CONSTANTS
             ...
             business_function TYPE dok_id VALUE 'BF',
             business_object   TYPE dok_id VALUE 'OD',
             extension_index TYPE dok_id VALUE 'XI',
           END OF gc_dok.

DATA: scwb_docu_class_support TYPE funcname VALUE 'SCWB_DOCU_CLASS_SUPPORT', "#EC NOTEXT   "will be cleared if function does not exist
...
DEFINE _addstring.
  concatenate gv_string &1 into gv_string
    ##STRING_OK        ">=SAPBASIS740    pragma as of 720
   .                                                        "#EC NOTEXT
END-OF-DEFINITION.
...
*  "Initialize texts for selection screen 1900                                "#EC NOTEXT   "1900
  ...
  CALL FUNCTION 'FUNCTION_EXISTS'
    EXPORTING
      funcname           = 'SCWB_DOCU_CLASS_SUPPORT'
    EXCEPTIONS
      function_not_exist = 4
      OTHERS             = 0.
  IF sy-subrc NE 0.
    CLEAR scwb_docu_class_support.
  ENDIF.
...
*                       with USER
      ...
      ELSEIF c_state-ucomm EQ '&IC1' AND c_state-list_field = 'OBJECT' AND ls_tadir-object IS NOT INITIAL.  "double click on object type
        ...
        ENDIF.
        c_state-ucomm_exec = 'X'.
      ELSEIF ls_msg-msgid EQ '00' AND ls_msg-msgno EQ '398' AND ls_msg-msgv1 EQ 'Object with delivery method'.
        PERFORM show_url IN PROGRAM (myrepid) USING 'https://wiki.wdf.sap.corp/wiki/display/~d028497/Delivery+Methods' IF FOUND.
        c_state-ucomm_exec = 'X'.
      ELSEIF ls_msg-msgid EQ '00' AND ls_msg-msgno EQ '398'.
        CHECK gc_objtext-longtext NE space.   "dummy for where-used-list
        CASE ls_tadir-object.
          ...
          WHEN 'PROG'.
            ...
            c_state-ucomm_exec = 'X'.
          WHEN 'TOBJ'.
            PERFORM show_url IN PROGRAM (myrepid) USING 'https://wiki.wdf.sap.corp/wiki/display/~d028497/TOBJ' IF FOUND. "Wiki TOBJ
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS create_table    "or structure or append or database table
      IMPORTING
        ...
        i_skip_snote_check TYPE xfeld OPTIONAL
    ...
    CLASS-METHODS set_context
      IMPORTING i_object     TYPE e071-object
                ...
                i_masterlang TYPE masterlang OPTIONAL    "obligatory for SNOTE objects!
    ...
    CLASS-METHODS snote_support
      IMPORTING i_object  TYPE e071-object
                i_objname TYPE e071-obj_name OPTIONAL
...
CLASS lcl_wb IMPLEMENTATION.
  ...
  METHOD init.
    ...
    CONCATENATE 'AOBJ''CHDO''IWPR''IWMO''IWSV''SUCD''SUSC''SUSK''DIOB''DTRF''DROU'
                ...
                'UIAC''UIAD''ADVC'   "new since SCWB does not work properly
                'STYC''DAQR''ESH2''SRFP''UISC''UIPC''AIFC''SITU''SITD''SITO''SITB''SITC'        "pilot
    ...
    IF sy-cprog NE myrepid.
      ...
      SELECT SINGLE unam cnam udat cdat FROM d010sinf INTO CORRESPONDING FIELDS OF ls_d010sinf WHERE prog EQ sy-repid. "#EC CI_NOORDER
      ...
      IF ls_d010sinf-cdat < l_olded AND ls_d010sinf-udat < l_olded AND gv_unit_test IS INITIAL.
        "If customer used user SAP to apply that report they have to comment out the following 4 lines:
        MESSAGE s666(01) WITH 'This report was implemented more than 60 days ago'  'and shall not be executed anymore.' INTO sy-lisel. "#EC *
        CALL METHOD log_message.
        r_return_code = 3.
        RETURN.
      ENDIF.
...
*------
  ...
  METHOD create_table.
    ...
    IF lcl_wb=>snote_support( 'TABL' ) IS INITIAL OR i_skip_snote_check IS NOT INITIAL.
...
*       GOTSTATE      = GOTSTATE
    ...
    CALL METHOD add_to_transport
      EXPORTING
        ...
        i_edtflag          = i_edtflag
        i_skip_snote_check = i_skip_snote_check
      EXCEPTIONS
        OTHERS             = 1.
    CHECK sy-subrc EQ 0.
  ...
  METHOD activate.
    ...
    IF testrun IS INITIAL.
      ...
      FIELD-SYMBOLS: <lt_upginfo> TYPE STANDARD TABLE, <ls_upginfo> TYPE any, <l_putstatus> TYPE char1.
      DATA: upg_get_upgrade_info TYPE funcname VALUE 'UPG_GET_UPGRADE_INFO',
            dref TYPE REF TO data.
      CREATE DATA dref TYPE TABLE OF ('UVERS').
      ASSIGN dref->* TO <lt_upginfo>.
      ...
      READ TABLE <lt_upginfo> ASSIGNING <ls_upginfo> INDEX 1.
      ASSIGN COMPONENT 'PUTSTATUS' OF STRUCTURE <ls_upginfo> TO <l_putstatus>.
      IF sy-subrc EQ 0 AND ( <l_putstatus> = 'S' OR <l_putstatus> = 'U' ).
    ...
    IF sy-subrc > 4.  "4 is only warning
      ...
      LOOP AT lt_log INTO ls_log WHERE severity EQ 'E' OR newsection EQ 'X'.
        IF ls_log-newsection EQ 'X'.
          ...
          ELSE.
            MESSAGE i350(ecatt) WITH ls_log-line(50) ls_log-line+50(50) ls_log-line+100 INTO sy-lisel. "#EC *
          ENDIF.
          CALL METHOD set_context
            EXPORTING
              i_masterlang = space
        ...
        IF ls_log-severity EQ 'E'.
          MESSAGE e350(ecatt) WITH ls_log-line(50) ls_log-line+50(50) ls_log-line+100 INTO sy-lisel. "#EC *
          CALL METHOD log_message.
        ENDIF.
      ENDLOOP.
      RAISE error.
...
*    CALL FUNCTION 'APPEND_PROTOCOL'.    "here error occurs depending on previous statements
    ...
    IF sy-saprl >= '700'.
      ...
      CALL FUNCTION trint_call_after_imp_method
        ...
        EXCEPTIONS
          error_message      = 1.
    ELSE.
      APPEND sy-mandt TO lt_client.                   "#EC CI_APPEND_OK
      CALL FUNCTION trint_call_after_imp_method
        EXPORTING
          iv_trkorr          = space
          is_e070            = ls_e070
    ...
    LOOP AT lt_trlog INTO ls_trlog.
      ...
      MESSAGE i350(ecatt)) WITH ls_trlog-line(50) ls_trlog-line+50(50) ls_trlog-line+100(31) sy-msgv4 INTO sy-lisel. "#EC *
      ...
      ENDIF.
      TRANSLATE ls_trlog-severity USING ' I'. MOVE ls_trlog-severity TO sy-msgty.
      CALL METHOD log_message.
    ENDLOOP.
...
*
    ...
    TRY.
        IF cl_abap_dyn_prg=>check_column_name( i_tabname ) IS INITIAL.
          MESSAGE e666(01) WITH 'Please implement note 1717578.' INTO sy-msgli. "#EC *
          CALL METHOD lcl_wb=>log_message.
          RETURN.
        ENDIF.
      CATCH cx_root.
    ...
    IF ls_dd02v IS INITIAL.
      CALL METHOD lcl_wb=>set_context( i_object = 'TABU' i_obj_name = i_tabname i_masterlang = space i_langu = space i_no_message = 'X' ).
      ...
      ENDIF.
      CALL METHOD lcl_wb=>log_message.
      CALL METHOD lcl_wb=>set_context( i_object = space i_obj_name = space i_masterlang = space i_langu = space ).
      RETURN.
...
*        EXIT.    "do not EXIT and trigger "manual/BCSet"
    ...
    IF ls_dd02v-contflag CA 'SE'. "do not allow W!   R3TR TABU fpr W table should not appear in transport  example: ERFK180071
      ...
      LOOP AT lt_dd03p TRANSPORTING NO FIELDS WHERE ( datatype    NE 'CLNT' AND
                                                      ...
                                                      datatype    NE 'TIMS' AND  "Time in format HHMMSS
                                                      datatype    NE 'TIMN' AND  "Time in format HHMMSS (native HANA type TIME)
                                                      datatype    NE 'LANG' AND
                                                      datatype    NE 'DATS' AND  "Date in format YYYYMMDD
                                                      datatype    NE 'DATN' AND  "Date in format YYYYMMDD (native HANA type DATE)
                                                      datatype    NE 'DEC'  AND  " works with = '500.12 ' (independent from sy-langu)
                                                NOT ( datatype    EQ 'RAW'  AND leng EQ '000016' ) AND "RAW 16 usually a GUID which works fine, used in 'BOPX'
                                                      datatype    NE 'SSTR' AND "SSTRING is STRING with max lan of 255
                                                      datatype    NE 'STRG' AND
                                                      datatype(3) NE 'INT'
                                                       )
                                                 OR ( outputlen > 255 ).
        EXIT.
      ENDLOOP.
      IF sy-subrc NE 0.  "no datatype which is not allowed
        l_approved_system_table = 'X'.   "basically allowed
        LOOP AT lt_dd03p TRANSPORTING NO FIELDS WHERE ( keyflag EQ 'X' )    AND
                                                      ( datatype  NE 'CLNT' AND
                                                        datatype  NE 'CHAR' AND
                                                        datatype  NE 'LANG' AND
                                                        datatype  NE 'NUMC' AND
                                                        datatype  NE 'DATS' AND
                                                        datatype  NE 'DATN' AND
                                                  NOT ( datatype  EQ 'RAW'  AND leng EQ '000016' ) ). "RAW 16 usually a GUID which works fine, used in 'BOPX'
          l_approved_system_table = space.    "invalid datatype in key fields
          EXIT.
        ENDLOOP.
      ENDIF.
    ENDIF.
...
*           'EDIFCT' OR 'TBDBA' OR 'TBDBE' OR 'TBDBH' OR 'TBDBS' OR       "TDAT BDBG
           ...
           'TBCA_PAY_GA_SPLT' OR 'TBCA_PAY_GA_S_T' OR
            ...
            '' OR 'T185D' OR '' OR 'T185F' OR 'T185V' OR '' OR           "VFBS
            'TC02' OR

           '' OR                                       "PPH

           'TC33' OR 'TC34' OR '' OR                               "Status handling

           'TMC73' OR

           'FINB_TR_TCAT' OR 'FINB_TR_TCATT' OR 'FINB_TR_TAPPL' OR 'FINB_TR_TAPPLT' OR

           'TXW_C_SOEX' OR 'TXW_C_STRC' OR '' OR
           ...
           '/BA1/F0_LOC_FIX'  OR
            ...
            '/BA1/FF_ALV_LAYO' OR                           "F8DK039680
            '/BA1/FF_ALV_LAYT' OR                           "F8DK039680

           '/FSPD/TCDIR_CAGT' OR
           '/FSPD/TCDIR_CATG' OR

           '/AIF/PERS_DB_MAP' OR                            "ER9K8AO11A
           '/AIF/PERS_DB_PRE' OR                     "wrong table category, shall be handled like 'S'
        ...
        IF l_approved_system_table EQ 'X'.
*           .
        ENDIF.
...
*        PERFORM prepare_table USING i_tabname 'IDOCTYP LANGUA' it_entries CHANGING l_no_update.
      ...
      WHEN 'STC_SCN_HDR' OR 'STC_SCN_TASKS' OR 'STC_SCN_ATTR'.                       "STCS   -> keep for old releases
...
*        ls_e071k-tabkey = <l_tabkey>(l_keylen).
      LOOP AT it_entries ASSIGNING <ls_entry>.
        ...
        ENDIF.
        TRY.                                                                                    ">=SAPBASIS700
            ls_e071k-tabkey = ls_e071k-tabkey(l_keylen).
          CATCH cx_root.  "complex datatype v, e.g. because of string                           ">=SAPBASIS700
        ENDTRY.                                                                                 ">=SAPBASIS700
        IF ls_e071k-tabkey IS INITIAL.  "above easy MOVE did not work
          LOOP AT lt_dfies TRANSPORTING NO FIELDS WHERE keyflag EQ 'X'.
            ASSIGN COMPONENT sy-tabix OF STRUCTURE <ls_entry> TO <l_field>.
            DESCRIBE DISTANCE BETWEEN <ls_entry> AND <l_field> INTO sy-fdpos IN CHARACTER MODE.
            ls_e071k-tabkey+sy-fdpos = <l_field>.          "CONCATENATE key fields
          ENDLOOP.
        ENDIF.
        IF l_keylen <= 120.
...
*             TT_TADIR                =
          ...
          IF sy-subrc EQ 0.
            IF l_trkorr IS INITIAL.
              CHECK lcl_wb=>continue_without_transport( ) NE space.
  ...
  METHOD add_to_transport.
    ...
    ELSEIF i_skip_snote_check IS INITIAL AND lcl_wb=>snote_support( i_object ) NE space.
...
*                DELETE FROM tadir WHERE pgmid EQ ls_tadir_key-pgmid AND object EQ ls_tadir_key-object AND obj_name   EQ ls_tadir_key-obj_name.
    ...
    IF testrun IS INITIAL AND gv_trkorr IS INITIAL AND i_devclass NE space AND i_devclass NE '$TMP'. " IF lcl_internal=>local_object( is_object-devclass ) = 'X'.   "#EC NOTEXT
...
*          OTHERS             = 3.
      ...
      SELECT * FROM balhdr "#EC CI_ALL_FIELDS_NEEDED  "#EC CI_BUFFSUBQ   "read from all clients to get really all logs for this report
             CLIENT SPECIFIED INTO TABLE lt_log_header WHERE object EQ c_bal_object AND subobject EQ c_bal_subobj AND extnumber EQ sy-repid. "#EC CI_CLIENT
      IF sy-subrc EQ 0.
    ...
    IF NOT i_no_save IS INITIAL.
      CALL FUNCTION 'BAL_LOG_REFRESH'
        ...
        EXCEPTIONS
          OTHERS       = 0.
      "reset context
      CALL METHOD set_context( EXPORTING i_masterlang = space i_langu = space i_object = space i_obj_name = space ).
    ENDIF.
  ...
  METHOD set_context.
    ...
    IF NOT i_object IS INITIAL AND NOT i_obj_name IS INITIAL.
      ls_e071-pgmid    = 'R3TR'.
      ls_e071-object   = i_object.
      CONCATENATE i_obj_name i_obj_name2 INTO ls_e071-obj_name.
...
*          CALL METHOD log_message.   "confuses most users without real benefit
        ENDIF.
      ENDIF.
    ENDIF.
    ...
    IF l_text NE space.
      ...
      ELSEIF i_skip_snote_check NE space.
        "SNOTE_CHECK suppressed
      ELSEIF i_object EQ 'DOCU' AND lcl_wb=>snote_support( i_object = i_object i_objname = ls_e071-obj_name ) NE space OR
             lcl_wb=>snote_support( i_object ) NE space.
        IF i_langu NE space AND i_masterlang IS INITIAL.
          MESSAGE w666(01) WITH 'Master language missing' INTO sy-lisel. "#EC *
          CALL METHOD log_message.
        ENDIF.
  ...
  METHOD snote_support. "check if supported in current customer system

    STATICS: s_note_2459558.
    DATA: l_object  LIKE i_object,
          l_objname TYPE e071-obj_name.
    ...
    IF l_object EQ 'DOCU' AND scwb_docu_class_support IS NOT INITIAL.
      CALL FUNCTION scwb_docu_class_support
        EXPORTING
          iv_objname = i_objname
        IMPORTING
          ev_snote   = r_supported.
      RETURN.
    ENDIF.
...
CLASS ltc_testrun IMPLEMENTATION.                                ">=SAPBASIS702
  METHOD execute.                                                ">=SAPBASIS702
    ...
    CHECK sy-saprl GE gc_sapbasis_infinity. "full test only in infinity   ">=SAPBASIS702
...
FORM main.
  ...
  DO.                                                           "1900
    ...
    IF sy-cprog NE myrepid AND gv_unit_test IS INITIAL.
      SUBMIT (sy-repid) "#EC CI_SUBMIT  "start from scratch with initial global variables
...
FORM update_01. " TABU TRESC ( DE , source: EV8 )
  TYPES: BEGIN OF ty_tresc,
          object(4),
          tabname(30),
          fieldname(30),
          keylow(30),
          ...
          lastdate TYPE D,
        END OF ty_tresc.
  DATA: lt_tresc TYPE TABLE OF ty_tresc.
  FIELD-SYMBOLS: <ls_tresc> LIKE LINE OF lt_tresc.

  APPEND INITIAL LINE TO lt_tresc ASSIGNING <ls_tresc>.
  <ls_tresc>-object       = `TABU`.                                                                 "#EC NOTEXT
  <ls_tresc>-tabname      = `N1POOBJCTDISPT`.                                                       "#EC NOTEXT
  <ls_tresc>-fieldname    = `DISPLAY_ID`.                                                           "#EC NOTEXT
  <ls_tresc>-keylow       = `/*`.                                                                   "#EC NOTEXT
  ...
  <ls_tresc>-object       = `TABU`.                                                                 "#EC NOTEXT
  <ls_tresc>-tabname      = `N1POOBJCTDISPT`.                                                       "#EC NOTEXT
  <ls_tresc>-fieldname    = `OBJECTTYPE_ID`.                                                        "#EC NOTEXT
  <ls_tresc>-keylow       = `/*`.                                                                   "#EC NOTEXT
  <ls_tresc>-lastname     = sy-uname.                                                               "#EC NOTEXT
  <ls_tresc>-lastdate     = sy-datum.                                                               "#EC NOTEXT
  ...
  APPEND INITIAL LINE TO lt_tresc ASSIGNING <ls_tresc>.
  <ls_tresc>-object       = `TABU`.                                                                 "#EC NOTEXT
  <ls_tresc>-tabname      = `N1POOBJCTTYPDISP`.                                                     "#EC NOTEXT
  <ls_tresc>-fieldname    = `DISPLAY_ID`.                                                           "#EC NOTEXT
  <ls_tresc>-keylow       = `/*`.                                                                   "#EC NOTEXT
  <ls_tresc>-lastname     = sy-uname.                                                               "#EC NOTEXT
  <ls_tresc>-lastdate     = sy-datum.                                                               "#EC NOTEXT
  ...
  APPEND INITIAL LINE TO lt_tresc ASSIGNING <ls_tresc>.
  <ls_tresc>-object       = `TABU`.                                                                 "#EC NOTEXT
  <ls_tresc>-tabname      = `N1POOBJECTTYPE`.                                                       "#EC NOTEXT
  <ls_tresc>-fieldname    = `ID`.                                                                   "#EC NOTEXT
  <ls_tresc>-keylow       = `/*`.                                                                   "#EC NOTEXT
  <ls_tresc>-lastname     = sy-uname.                                                               "#EC NOTEXT
  <ls_tresc>-lastdate     = sy-datum.                                                               "#EC NOTEXT
  ...
  APPEND INITIAL LINE TO lt_tresc ASSIGNING <ls_tresc>.
  <ls_tresc>-object       = `TABU`.                                                                 "#EC NOTEXT
  <ls_tresc>-tabname      = `N1POOBJECTTYPET`.                                                      "#EC NOTEXT
  <ls_tresc>-fieldname    = `ID`.                                                                   "#EC NOTEXT
  <ls_tresc>-keylow       = `/*`.                                                                   "#EC NOTEXT
  <ls_tresc>-lastname     = sy-uname.                                                               "#EC NOTEXT
  <ls_tresc>-lastdate     = sy-datum.                                                               "#EC NOTEXT
...
FORM update.
  ...
  PERFORM update_01. " TABU TRESC ( DE , source: EV8 )
