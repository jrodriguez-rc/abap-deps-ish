class CL_EX_N1_CONNECTIVITY definition
  public
  final
  create public .

*"* public components of class CL_EX_N1_CONNECTIVITY
*"* do not include other source files here!!!
public section.

  interfaces IF_EX_N1_CONNECTIVITY .

  constants VERSION type VERSION value 000001. "#EC NOTEXT
  type-pools SXRT .
protected section.
*"* protected components of class CL_EX_N1_CONNECTIVITY
*"* do not include other source files here!!!
private section.
*"* private components of class CL_EX_N1_CONNECTIVITY
*"* do not include other source files here!!!
...
CLASS CL_EX_N1_CONNECTIVITY IMPLEMENTATION.


method IF_EX_N1_CONNECTIVITY~EXIT_CHECK.
  CLASS CL_EXIT_MASTER DEFINITION LOAD.
  DATA: EXIT_OBJ_TAB TYPE SXRT_EXIT_TAB.

  DATA: exitintf TYPE REF TO IF_EX_N1_CONNECTIVITY,
  ...
  IF sy-subrc NE 0.
    ...
    <flt_cache_line>-method_name = 'EXIT_CHECK'.
        ...
        CALL METHOD CL_EXIT_MASTER=>CREATE_OBJ_BY_INTERFACE_FILTER
           EXPORTING
              CALLER       = me
              INTER_NAME   = 'IF_EX_N1_CONNECTIVITY'
              METHOD_NAME  = 'EXIT_CHECK'
  ...
  IF <flt_cache_line>-valid = sxrt_true.
    ...
    CALL FUNCTION 'PF_ASTAT_OPEN'
       EXPORTING
           OPENKEY = 'Gb8uoVri9NhX00002XLK0G'
           TYP     = 'UE'.
    ...
    CASE <flt_cache_line>-imp_switch.
      WHEN 'VSR'.
        ...
        IF <flt_cache_line>-eo_object is initial.
          CALL METHOD ('CL_FOBU_METHOD_EVALUATION')=>load
               EXPORTING
                  ...
                  im_interface_name = 'IF_EX_N1_CONNECTIVITY'
                  im_method_name    = 'EXIT_CHECK'
               RECEIVING
        ...
        CLEAR data_ref.
        GET REFERENCE OF I_CALLER INTO data_ref.
        CALL METHOD <flt_cache_line>-eo_object->set_parameter(
            im_parmname = 'I_CALLER'
            im_value    = data_ref ).

        CLEAR data_ref.
        GET REFERENCE OF IT_OBJECTS INTO data_ref.
        CALL METHOD <flt_cache_line>-eo_object->set_parameter(
            im_parmname = 'IT_OBJECTS'
            im_value    = data_ref ).
      ...
      WHEN OTHERS.
        ...
        CALL METHOD EXITINTF->EXIT_CHECK
           EXPORTING
             I_CALLER = I_CALLER
...
method IF_EX_N1_CONNECTIVITY~EXIT_EXPORT.
  CLASS CL_EXIT_MASTER DEFINITION LOAD.
  DATA: EXIT_OBJ_TAB TYPE SXRT_EXIT_TAB.
  ...
  IF sy-subrc NE 0.
    ...
    ASSIGN wa_flt_cache_line->* TO <flt_cache_line>.
    <flt_cache_line>-flt_name    = flt_name.
    <flt_cache_line>-method_name = 'EXIT_EXPORT'.
  ...
  IF <flt_cache_line>-valid = sxrt_true.
    ...
    CALL FUNCTION 'PF_ASTAT_OPEN'
       EXPORTING
           OPENKEY = 'Gb8up}ri9NhX00002XLK0G'
           TYP     = 'UE'.
    ...
    CASE <flt_cache_line>-imp_switch.
      ...
      WHEN OTHERS.
        EXITINTF ?= <flt_cache_line>-OBJ.
        CALL METHOD EXITINTF->EXIT_EXPORT
           EXPORTING
             I_CALLER = I_CALLER
             IT_OBJECTS = IT_OBJECTS
           IMPORTING
...
method IF_EX_N1_CONNECTIVITY~EXIT_IMPORT_CORDER.
  CLASS CL_EXIT_MASTER DEFINITION LOAD.
  DATA: EXIT_OBJ_TAB TYPE SXRT_EXIT_TAB.
  ...
  READ TABLE INSTANCE_FLT_CACHE ASSIGNING <flt_cache_line>
         WITH KEY flt_name    = flt_name
                  method_name = 'EXIT_IMPORT_CORDER'
         .
  IF sy-subrc NE 0.
    ...
    ASSIGN wa_flt_cache_line->* TO <flt_cache_line>.
    <flt_cache_line>-flt_name    = flt_name.
    <flt_cache_line>-method_name = 'EXIT_IMPORT_CORDER'.
  ...
  IF <flt_cache_line>-valid = sxrt_true.
    ...
    CALL FUNCTION 'PF_ASTAT_OPEN'
       EXPORTING
           OPENKEY = 'Gb8urVri9NhX00002XLK0G'
           TYP     = 'UE'.
    ...
    CASE <flt_cache_line>-imp_switch.
      WHEN 'VSR'.
        ...
        IF <flt_cache_line>-eo_object is initial.
          CALL METHOD ('CL_FOBU_METHOD_EVALUATION')=>load
               EXPORTING
                  im_class_name     = <flt_cache_line>-imp_class
                  im_interface_name = 'IF_EX_N1_CONNECTIVITY'
                  im_method_name    = 'EXIT_IMPORT_CORDER'
               RECEIVING
                  re_fobu_method    = <flt_cache_line>-eo_object
               EXCEPTIONS
                  not_found         = 1
        ...
        CLEAR data_ref.
        GET REFERENCE OF IR_CORDER INTO data_ref.
        CALL METHOD <flt_cache_line>-eo_object->set_parameter(
            im_parmname = 'IR_CORDER'
            im_value    = data_ref ).

        CLEAR data_ref.
        GET REFERENCE OF IS_CORDER_DATA INTO data_ref.
        CALL METHOD <flt_cache_line>-eo_object->set_parameter(
            im_parmname = 'IS_CORDER_DATA'
            im_value    = data_ref ).
      ...
      WHEN OTHERS.
        EXITINTF ?= <flt_cache_line>-OBJ.
        CALL METHOD EXITINTF->EXIT_IMPORT_CORDER
           EXPORTING
             IR_CORDER = IR_CORDER
             IS_CORDER_DATA = IS_CORDER_DATA
           IMPORTING
             E_RC = E_RC
           CHANGING
             CR_ERRORHANDLER = CR_ERRORHANDLER.
