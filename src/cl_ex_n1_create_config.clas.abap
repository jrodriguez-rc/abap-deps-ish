class CL_EX_N1_CREATE_CONFIG definition
  public
  final
  create public .

*"* public components of class CL_EX_N1_CREATE_CONFIG
*"* do not include other source files here!!!
public section.

  interfaces IF_EX_N1_CREATE_CONFIG .

  constants VERSION type VERSION value 000001. "#EC NOTEXT
  type-pools SXRT .
protected section.
*"* protected components of class CL_EX_N1_CREATE_CONFIG
*"* do not include other source files here!!!
private section.
*"* private components of class CL_EX_N1_CREATE_CONFIG
*"* do not include other source files here!!!
...
CLASS CL_EX_N1_CREATE_CONFIG IMPLEMENTATION.


method IF_EX_N1_CREATE_CONFIG~CREATE_CONFIG.
  CLASS CL_EXIT_MASTER DEFINITION LOAD.
  DATA: EXIT_OBJ_TAB TYPE SXRT_EXIT_TAB.

  DATA: exitintf TYPE REF TO IF_EX_N1_CREATE_CONFIG,
  ...
  IF sy-subrc NE 0.
    ...
    <flt_cache_line>-method_name = 'CREATE_CONFIG'.
        ...
        CALL METHOD CL_EXIT_MASTER=>CREATE_OBJ_BY_INTERFACE_FILTER
           EXPORTING
              CALLER       = me
              INTER_NAME   = 'IF_EX_N1_CREATE_CONFIG'
              METHOD_NAME  = 'CREATE_CONFIG'
  ...
  IF <flt_cache_line>-valid = sxrt_true.
    ...
    CALL FUNCTION 'PF_ASTAT_OPEN'
       EXPORTING
           OPENKEY = 'GUO}X3zvH8}X00002XLK0G'
           TYP     = 'UE'.
    ...
    CASE <flt_cache_line>-imp_switch.
      WHEN 'VSR'.
        ...
        IF <flt_cache_line>-eo_object is initial.
          CALL METHOD ('CL_FOBU_METHOD_EVALUATION')=>load
               EXPORTING
                  ...
                  im_interface_name = 'IF_EX_N1_CREATE_CONFIG'
                  im_method_name    = 'CREATE_CONFIG'
               RECEIVING
        ...
        CLEAR data_ref.
        GET REFERENCE OF ER_CONFIG INTO data_ref.
        CALL METHOD <flt_cache_line>-eo_object->set_parameter(
            im_parmname = 'ER_CONFIG'
            im_value    = data_ref ).
      ...
      WHEN OTHERS.
        ...
        CALL METHOD EXITINTF->CREATE_CONFIG
           EXPORTING
