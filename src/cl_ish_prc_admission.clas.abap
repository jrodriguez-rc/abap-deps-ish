class CL_ISH_PRC_ADMISSION definition
  public
  create protected .

*"* public components of class CL_ISH_PRC_ADMISSION
...
public section.
  ...
  constants CO_OTYPE_PRC_ADMISSION type ISH_OBJECT_TYPE value 6006. "#EC NOTEXT
  ...
  class-methods CREATE
    exporting
      ...
      value(ER_INSTANCE) type ref to CL_ISH_PRC_ADMISSION
    changing
...
protected section.
*"* protected components of class CL_ISH_PRC_ADMISSION
*"* do not include other source files here!!!
  ...
  data GR_SCR_ADMISSION type ref to CL_ISH_SCR_ADMISSION .
  data GR_COMP_ADMISSION type ref to CL_ISH_COMP_ADMISSION .
  data GR_PRC_ADMISSION type ref to CL_ISH_PRC_ADMISSION .
  class-data G_COMP_ADMISSION_CLASSNAME type STRING .
  ...
  methods COMPLETE_CONSTRUCTION
    ...
    changing
      ...
      !CR_ERRORHANDLER type ref to CL_ISHMED_ERRORHANDLING optional .
private section.
*"* private components of class CL_ISH_PRC_ADMISSION
*"* do not include other source files here!!!
ENDCLASS.



CLASS CL_ISH_PRC_ADMISSION IMPLEMENTATION.
...
* Handle process environment.
  ...
  IF gr_environment IS INITIAL.
    CALL METHOD cl_ish_fac_environment=>create
      EXPORTING
        i_program_name = 'CL_ISH_PRC_ADMISSION'
      IMPORTING
...
* handle comp order
  CALL METHOD cl_ish_comp_admission=>create
    EXPORTING
      i_classname           = g_comp_admission_classname
...
*    I_CALLER              = SY-REPID
  ...
  IF NOT lr_instance IS INITIAL.
    IF lr_instance->is_inherited_from(
              cl_ish_comp_admission=>co_otype_comp_admission ) = on.
      gr_comp_admission ?= lr_instance.
    ENDIF.
  ENDIF.
  IF gr_comp_admission IS INITIAL.
    e_rc = 1.
    EXIT.
  ENDIF.

* get defined screens from comp order
  CALL METHOD gr_comp_admission->get_defined_screens
    RECEIVING
  ...
  IF sy-subrc = 0 AND
    ...
    IF lr_screen->is_inherited_from(
      ...
      gr_scr_admission ?= lr_screen.
    ENDIF.
  ENDIF.
  IF gr_scr_admission IS INITIAL.
    e_rc = 1.
  ENDIF.

* initialize the screen
  CALL METHOD gr_scr_admission->initialize
    EXPORTING
...
** Set dynpg/dynnr
*  g_dynpg = 'SAPLN1_SDY_ADMISSION'.
*  g_dynnr = '0100'.
* Handle component classnames.
  g_comp_admission_classname = 'CL_ISH_COMP_ADMISSION'.
...
* screen class
  CALL METHOD gr_scr_admission->set_instance_for_display.

* dynpro + program
  CALL METHOD gr_scr_admission->get_parent
    IMPORTING
      es_parent = gs_parent.
  es_parent = gs_parent.
...
METHOD CREATE .
  ...
  DATA: lr_prc_admission  TYPE REF TO cl_ish_prc_admission.

* Create the object.
  CREATE OBJECT lr_prc_admission.

* Complete construction.
  CALL METHOD lr_prc_admission->complete_construction
    IMPORTING
      e_rc            = e_rc
    CHANGING
...
METHOD IF_ISH_IDENTIFY_OBJECT~GET_TYPE .
  ...
  e_object_type = co_otype_prc_admission.
...
METHOD IF_ISH_IDENTIFY_OBJECT~IS_INHERITED_FROM .
  ...
  IF i_object_type = co_otype_prc_admission.
