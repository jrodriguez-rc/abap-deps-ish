...
* Generated by SAP_LOCAL_DOWNPORT_ASSISTANT Version 6.22 on 15.06.2017 with option ALL

* The individual coding starts at 'FORM update_01.' ( line 5.776 )
      ...
      ELSEIF ls_msg-msgid EQ '00' AND ls_msg-msgno EQ '398'.   "dummy message with long text icon
        CASE ls_tadir-object.
          ...
          WHEN OTHERS.  "'TABU', 'VIED', 'TABD', 'DEVC'
            PERFORM show_object_type IN PROGRAM (myrepid) USING ls_limu_key IF FOUND.  "-> special hints on object type page
            c_state-ucomm_exec = 'X'.
        ENDCASE.
      ENDIF.
  ENDCASE.
...
CLASS lcl_wb DEFINITION.
  PUBLIC SECTION.
    ...
    CLASS-METHODS create_index
      IMPORTING
        i_devclass      TYPE devclass
        i_tabname       TYPE tabname
        i_indexname     TYPE indexid
        i_text          TYPE ddtext OPTIONAL
        i_langu         TYPE sylangu DEFAULT 'E'
        i_isextind      TYPE xfeld OPTIONAL          "extension index
        i_uniqueflag    TYPE dd12l-uniqueflag OPTIONAL
        i_dbstate       TYPE dd12l-dbstate    OPTIONAL
        i_dbinclexcl    TYPE dd12l-dbinclexcl OPTIONAL
        i_dbsyssel1     TYPE dd12l-dbsyssel1  OPTIONAL
        i_dbsyssel2     TYPE dd12l-dbsyssel2  OPTIONAL
        i_dbsyssel3     TYPE dd12l-dbsyssel3  OPTIONAL
        i_dbsyssel4     TYPE dd12l-dbsyssel4  OPTIONAL
        is_fulltextinfo TYPE dd12fulltextinfo OPTIONAL  "new with 750?
        it_fields       TYPE tt_dd17v.
  ...
  METHOD init.
    ...
    IF sy-cprog(1) CA 'ZY'.  "check for not allowed namespace
      IF l_system_type EQ 'SAP'.
        MESSAGE e398(00) WITH 'Report will NOT work in customer system' 'because of wrong namespace (Z*)!' 'See longtext.' INTO sy-lisel. "#EC *
      ELSE.
        MESSAGE e398(00) WITH 'Report name is not in SAP namespace.' INTO sy-lisel. "#EC *
        r_return_code = 5. gv_errors_occured = 'X'.
        CALL METHOD log_message.
      ENDIF.
    ENDIF.
...
*       IV_NO_TADIR_OBJECT        = ' '
    ...
    DELETE lt_e071k WHERE objname EQ 'HRS1000' OR objname EQ 'HRS1206'   "'PDTS'  wrong langauge key-> skip deletion
...
*      APPEND INITIAL LINE TO lt_textpool ASSIGNING <ls_textpool>.
    ...
    TRY.                                                                                                               ">=SAPBASIS700
      ...
      CATCH cx_root.  "e.g. CX_SY_DYN_CALL_ILLEGAL_FUNC                                                                 ">=SAPBASIS700
        MESSAGE e666(01) WITH 'Error in function RPY_TEXTELEMENTS_INSERT.' 'Check Note 1842889' INTO sy-lisel. "#EC *   ">=SAPBASIS700
  ...
  METHOD create_index.
    ...
    DATA: l_obj_name  TYPE trobj_name,
          ...
          ls_indexes  LIKE LINE OF gr_indexes VALUE 'IEQ',
          ls_indxtab  LIKE LINE OF gr_indxtab VALUE 'IEQ',
          ls_indxname LIKE LINE OF gr_indxname VALUE 'IEQ'.

    IF i_isextind IS INITIAL.
      l_object = 'INDX'.
    ELSE.
      l_object = 'XINX'.
    ENDIF.

    "CONCATENATE i_table i_indexname INTO l_obj_name RESPECTING BLANKS.
    l_obj_name    = i_tabname.
    l_obj_name+30 = i_indexname.

    CALL FUNCTION 'SVRS_LONG2SHORT_NAME'     "could be useful for DYNP and MESS as well
      EXPORTING
        objtype       = l_object
        objname_long  = l_obj_name(110)    "cast to 110 character field
      IMPORTING
        objname_short = l_obj_name(110).

    ls_indexes-low  = l_obj_name.
    ls_indxtab-low  = i_tabname.
    ls_indxname-low = i_indexname.
...
* check object exists
    DATA: l_index_state TYPE ddgotstate.
    CALL FUNCTION 'DDIF_INDX_GET'
      EXPORTING
        name          = i_tabname
        id            = i_indexname
      IMPORTING
        gotstate      = l_index_state
      EXCEPTIONS
        illegal_input = 1
        OTHERS        = 2.
    IF sy-subrc <> 0.
    ...
    CALL METHOD add_to_transport
      EXPORTING
        i_object   = l_object        "INDX or XINX
        i_obj_name = l_obj_name
        i_devclass = i_devclass
        i_langu    = i_langu  "masterlanguage (only relevant during first call)
      EXCEPTIONS
    ...
    DATA: ls_index TYPE dd12v.
    ls_index-sqltab     = i_tabname.
    ls_index-indexname  = i_indexname.
    ls_index-ddtext     = i_text.
    ls_index-ddlanguage = i_langu.
    ls_index-dbstate    = i_dbstate.
    ls_index-uniqueflag = i_uniqueflag.
    ls_index-dbinclexcl = i_dbinclexcl.
    ls_index-dbsyssel1  = i_dbsyssel1.
    ls_index-dbsyssel2  = i_dbsyssel2.
    ls_index-dbsyssel3  = i_dbsyssel3.
    ls_index-dbsyssel4  = i_dbsyssel4.
    ls_index-isextind   = i_isextind.    ">=SAPBASIS700
    MOVE-CORRESPONDING is_fulltextinfo TO ls_index.
    ...
    CALL METHOD register_inactive_object
      EXPORTING
        i_objtype  = l_object
        i_objname  = l_obj_name
        i_treetype = space.

    COLLECT ls_indexes  INTO gr_indexes.
    COLLECT ls_indxtab  INTO gr_indxtab.
    COLLECT ls_indxname INTO gr_indxname.
...
FORM update_01. " FUGR 0N1TC ( DE , source: EV8 )
  CALL METHOD lcl_wb=>create_function_group   "#EC *
    EXPORTING
      ...
      i_langu      = 'D'
      i_appl       = 'S'
      i_text       = 'BehAuftrag Customizing'.

ENDFORM.

FORM update_02. " DTED N1TC_NO_CLEAR_BUFFER ( DE , source: EV8 )
  CALL METHOD lcl_wb=>create_dtel    "#EC *
    EXPORTING
      i_rollname  = 'N1TC_NO_CLEAR_BUFFER'
      i_devclass  = 'N1BASE'
      i_domname   = 'XFELD'
      i_headlen   = '55'
      i_scrlen1   = '10'
      ...
      i_langu     = 'D'
      i_ddtext    = 'Behandlungsauftrag: Puffer nicht automatisch leeren'
      i_reptext   = 'Puffer nicht automatisch leeren'
      i_scrtext_s = 'NichtLeer'
      i_scrtext_m = 'Puffer nicht leeren'
      i_scrtext_l = 'Puffer nicht automatisch leeren'.
ENDFORM.

FORM update_03. " DOCU DEN1TC_NO_CLEAR_BUFFER ( DE , source: EV8 )
  CALL METHOD lcl_wb=>create_dtel_docu     "#EC *
    EXPORTING
      ...
      i_line      =:'U1 &DEFINITION&',                                                                             "#EC NOTEXT
                    'AS Gibt an, dass die Daten in der Tabelle <LS>N1TC_BUFFER</> nicht',                          "#EC NOTEXT
                    '    automatisch gelöscht werden.,,',                                                          "#EC NOTEXT
                    'U1 &USE&',                                                                                    "#EC NOTEXT
                    'AS Sie können den Parameter verwenden, um die Systemgeschwindigkeit bei',                     "#EC NOTEXT
                    '   aktivierter Prüfung des Behandlungsauftrags zu verbessern.',                               "#EC NOTEXT
                    'U1 &RECOMMENDATION&',                                                                         "#EC NOTEXT
                    'AS <ZH>Beachten Sie, dass die Tabelle <LS>N1TC_BUFFER</> regelmäßig geleert',                 "#EC NOTEXT
                    '   werden muss.</> Verwenden Sie dazu den Report',                                            "#EC NOTEXT
                    '=   <DS:RE.RN1TC_CLEARBUFFER_BDC>RN1TC_CLEARBUFFER_BDC</>.',                                  "#EC NOTEXT
                    'U1 &DEPENDENCIES&',                                                                           "#EC NOTEXT
                    'AS Das System prüft diesen Parameter gemeinsam mit dem Benutzerparameter',                    "#EC NOTEXT
                    '   <LS>EIN</>.',                                                                              "#EC NOTEXT
...
FORM update_04. " TABT N1TC_BUFFER ( DE , source: EV8 )
  ...
  CALL METHOD lcl_wb=>create_table     "#EC *
    EXPORTING
      ...
      i_roworcolst = 'C'
      i_langu      = 'D'
      i_ddtext     = 'Tabelle für die speicherung von Prüfungen des Behandlungsauf'.

ENDFORM.

FORM update_05. " INDX N1TC_BUFFER                   2 ( DE , source: EV8 )
  DATA: lt_dd17v TYPE TABLE OF dd17v,
        ls_dd17v TYPE dd17v.

  CLEAR ls_dd17v.
  ls_dd17v-position              = '0001'.
  ls_dd17v-sqltab                = 'N1TC_BUFFER'.
  ls_dd17v-indexname             = '2'.
  ls_dd17v-fieldname             = 'MANDT'.
  APPEND ls_dd17v TO lt_dd17v.

  CLEAR ls_dd17v.
  ls_dd17v-position              = '0002'.
  ls_dd17v-sqltab                = 'N1TC_BUFFER'.
  ls_dd17v-indexname             = '2'.
  ls_dd17v-fieldname             = 'INSTITUTION_ID'.
  APPEND ls_dd17v TO lt_dd17v.

  CLEAR ls_dd17v.
  ls_dd17v-position              = '0003'.
  ls_dd17v-sqltab                = 'N1TC_BUFFER'.
  ls_dd17v-indexname             = '2'.
  ls_dd17v-fieldname             = 'PATIENT_ID'.
  APPEND ls_dd17v TO lt_dd17v.

  CLEAR ls_dd17v.
  ls_dd17v-position              = '0004'.
  ls_dd17v-sqltab                = 'N1TC_BUFFER'.
  ls_dd17v-indexname             = '2'.
  ls_dd17v-fieldname             = 'UNAME'.
  APPEND ls_dd17v TO lt_dd17v.

  CLEAR ls_dd17v.
  ls_dd17v-position              = '0005'.
  ls_dd17v-sqltab                = 'N1TC_BUFFER'.
  ls_dd17v-indexname             = '2'.
  ls_dd17v-fieldname             = 'INSERT_DATE'.
  APPEND ls_dd17v TO lt_dd17v.


  CALL METHOD lcl_wb=>create_index     "#EC *
    EXPORTING
      i_devclass   = 'N1BASE'
      i_indexname  = '2'
      i_tabname    = 'N1TC_BUFFER'
      i_langu      = 'D'
      i_text       = 'Index für API-Zugriff'
      i_dbstate    = 'D'
      i_dbinclexcl = 'E'
      i_dbsyssel1  = 'HDB'
      it_fields    = lt_dd17v.
...
FORM update_06. " TABT N1TC_BUFFER_PAP ( DE , source: EV8 )
  ...
  CALL METHOD lcl_wb=>create_table     "#EC *
    EXPORTING
      ...
      i_roworcolst = 'C'
      i_langu      = 'D'
      i_ddtext     = 'Tabelle Prüfungen des BA für vorläufige Patienten'.

ENDFORM.

FORM update_07. " TABT N1TC_MATRIX_BUF ( DE , source: EV8 )
  ...
  CALL METHOD lcl_wb=>create_table     "#EC *
    EXPORTING
      ...
      i_roworcolst = 'C'
      i_langu      = 'D'
      i_ddtext     = 'Zwischenspeicher der Matrix'.

ENDFORM.

FORM update_08. " TABD TN1TC_GLOB_SET ( DE , source: EV8 )
  ...
  CALL METHOD lcl_wb=>create_table     "#EC *
    EXPORTING
      ...
      i_roworcolst = 'C'
      i_langu      = 'D'
      i_ddtext     = 'Behandlungsauftrag - Allgemeine Einstellungen'.

  CLEAR: ls_dd08v, lt_dd05m.
  ls_dd08v-tabname               = 'TN1TC_GLOB_SET'.
  ...
  CLEAR ls_dd05m.
  ls_dd05m-tabname               = 'TN1TC_GLOB_SET'.
  ls_dd05m-fieldname             = 'MANDT'.
  ls_dd05m-fortable              = 'TN1TC_GLOB_SET'.
  ...
  CALL METHOD lcl_wb=>add_field_to_table    "#EC *
    EXPORTING
      ...
      is_dd08v    = ls_dd08v
      it_dd05m    = lt_dd05m
      i_context   = 'TN1TC_GLOB_SET'.

  CLEAR: ls_dd08v, lt_dd05m.
  ls_dd08v-tabname               = 'TN1TC_GLOB_SET'.
  ls_dd08v-fieldname             = 'INSTITUTION_ID'.
  ls_dd08v-tabname               = 'TN1TC_GLOB_SET'.
  ls_dd08v-fieldname             = 'INSTITUTION_ID'.
  ls_dd08v-checktable            = 'TN01'.
  ls_dd08v-frkart                = 'KEY'.
  ls_dd08v-card                  = 'CN'.
  ls_dd08v-cardleft              = '1'.
  ...
  CLEAR ls_dd05m.
  ls_dd05m-tabname               = 'TN1TC_GLOB_SET'.
  ls_dd05m-fieldname             = 'INSTITUTION_ID'.
  ls_dd05m-fortable              = 'TN1TC_GLOB_SET'.
  ls_dd05m-forkey                = 'MANDT'.
  ls_dd05m-checktable            = 'TN01'.
  ls_dd05m-checkfield            = 'MANDT'.
  ls_dd05m-primpos               = '0001'.
  APPEND ls_dd05m TO lt_dd05m.
  ...
  ls_dd05m-fieldname             = 'INSTITUTION_ID'.
  ls_dd05m-fortable              = 'TN1TC_GLOB_SET'.
  ls_dd05m-forkey                = 'INSTITUTION_ID'.
  ls_dd05m-checktable            = 'TN01'.
  ls_dd05m-checkfield            = 'EINRI'.
  ...
  CALL METHOD lcl_wb=>add_field_to_table    "#EC *
    EXPORTING
      i_tabname   = 'TN1TC_GLOB_SET'
      i_fieldname = 'SECURITY_LEVEL'
      i_rollname  = 'N1TC_SECURITY_LEVEL'
  ...
  CALL METHOD lcl_wb=>add_field_to_table    "#EC *
    EXPORTING
      i_tabname   = 'TN1TC_GLOB_SET'
      i_fieldname = 'DECAY_TIME'
      i_rollname  = 'N1TC_DECAY_TIME'
      i_context   = 'SECURITY_LEVEL'.
  ...
  CALL METHOD lcl_wb=>add_field_to_table    "#EC *
    EXPORTING
      i_tabname   = 'TN1TC_GLOB_SET'
      i_fieldname = 'NO_CLEAR_BUFFER'
      i_rollname  = 'N1TC_NO_CLEAR_BUFFER'
      i_context   = 'UPUSR'.

ENDFORM.

FORM update_09. " TABT TN1TC_GLOB_SET ( DE , source: EV8 )
...
FORM update_10. " VIED V_TN1TC_GLOB_SET ( DE , source: EV8 )
  DATA: ls_dd26v TYPE dd26v, lt_dd26v TYPE TABLE OF dd26v.

  CLEAR ls_dd26v.
  ls_dd26v-viewname       = 'V_TN1TC_GLOB_SET'.                                                     "#EC NOTEXT
  ls_dd26v-tabname        = 'TN1TC_GLOB_SET'.                                                       "#EC NOTEXT
  ls_dd26v-fortabname     = 'TN1TC_GLOB_SET'.                                                       "#EC NOTEXT
  APPEND ls_dd26v TO lt_dd26v.

  CLEAR ls_dd26v.
  ls_dd26v-viewname       = 'V_TN1TC_GLOB_SET'.                                                     "#EC NOTEXT
  ls_dd26v-tabname        = 'TN01'.                                                                 "#EC NOTEXT
  ls_dd26v-fortabname     = 'TN1TC_GLOB_SET'.                                                       "#EC NOTEXT
  ls_dd26v-forfield       = 'INSTITUTION_ID'.                                                       "#EC NOTEXT
  ls_dd26v-fordir         = 'E'.                                                                    "#EC NOTEXT
  APPEND ls_dd26v TO lt_dd26v.
  ...
  CALL METHOD lcl_wb=>create_view     "#EC *
    EXPORTING
      i_viewname   = 'V_TN1TC_GLOB_SET'
      i_devclass   = 'N1BASE'
      i_langu      = 'D'
      i_ddtext     = 'BehAuftrag Allgemeine Einstellungen'
      i_aggtype    = 'V'
      i_roottab    = 'TN1TC_GLOB_SET'
      i_viewclass  = 'C'
      i_viewgrant  = ''
      i_globalflag = 'X'
      it_dd26v     = lt_dd26v[]
...
FORM update_11. " REPT SAPL0N1TC ( DE , source: EV8 )
  CALL METHOD lcl_wb=>create_program_text    "#EC *
    EXPORTING
      i_progname  = 'SAPL0N1TC'
      i_devclass  = 'N1BASE'
      i_langu     = 'D':
      i_id        = 'I'
      i_key       = '001'
      i_text      = 'OE-Konstellationen'  "#EC NOTEXT
      i_length    = '30'.

ENDFORM.

FORM update_12. " REPT RN1TC_CLEARBUFFER_BDC ( DE , source: EV8 )
  CALL METHOD lcl_wb=>create_program_text    "#EC *
    EXPORTING
...
FORM update_13. " DOCU RERN1TC_CLEARBUFFER_BDC ( DE , source: EV8 )
  CALL METHOD lcl_wb=>create_other_docu     "#EC *
    EXPORTING
      i_devclass  = 'N1BASE'
      i_docname   = 'RERN1TC_CLEARBUFFER_BDC'
      i_langu     = 'D'
      i_line      =:'U1 &PURPOSE&',                                                                                "#EC NOTEXT
                    'AS Mit diesem Report können Sie in den Puffertabellen alle Daten löschen,',                   "#EC NOTEXT
                    '   die bei den Prüfungen des Behandlungsauftrages im System gesichert',                       "#EC NOTEXT
                    '   wurden.',                                                                                  "#EC NOTEXT
                    ...
                    'AS',                                                                                          "#EC NOTEXT
                    'U2 &OUTPUT&',                                                                                 "#EC NOTEXT
                    'AS Die Ergebnisse werden im Anwendungslog protokolliert. Wählen Sie in',                      "#EC NOTEXT
                    '   <LS>Anwendungslog auswerten</> (Transaktion SLG1) das Objekt <LS>ISHMED',                  "#EC NOTEXT
                    '=  </>, das Subobjekt <LS>REPORT</> und die externe Identifikation',                          "#EC NOTEXT
                    '   <LS>RN1TC_CLEARBUFFER_BDC</>.',                                                            "#EC NOTEXT
                    'U1 &ACTIVITIES&',                                                                             "#EC NOTEXT
                    'AS Um die Daten in den Puffertabellen regelmäßig zu löschen, planen Sie den',                 "#EC NOTEXT
                    '   Report in einem Batch-Job ein.',                                                           "#EC NOTEXT
                    'U1 &EXAMPLE&',                                                                                "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    space.
ENDFORM.

FORM update_14_0100. " DYNP SAPL0N1TC                               0100 ( DE EV8 )
  DATA: ls_rpy_dyhead TYPE rpy_dyhead, lt_rpy_dyhead TYPE TABLE OF rpy_dyhead.

  CLEAR ls_rpy_dyhead.
  ls_rpy_dyhead-program        = 'SAPL0N1TC'.                                                       "#EC NOTEXT
  ls_rpy_dyhead-screen         = '0100'.                                                            "#EC NOTEXT
  ls_rpy_dyhead-language       = 'D'.                                                               "#EC NOTEXT
  ls_rpy_dyhead-descript       = 'Viewpflege: Übersichtsbild V_TN1TC_GLOB_SET'.                     "#EC NOTEXT
  ls_rpy_dyhead-type           = 'N'.                                                               "#EC NOTEXT
  ls_rpy_dyhead-nextscreen     = '0100'.                                                            "#EC NOTEXT
  ...
  ls_rpy_dyfatc-cont_name        = 'TCTRL_V_TN1TC_GLOB_SET'.                                        "#EC NOTEXT
  ls_rpy_dyfatc-type             = 'TEMPLATE'.                                                      "#EC NOTEXT
  ...
  ls_rpy_dyfatc-name             = '*V_TN1TC_GLOB_SET-INSTITUTION_ID'.                              "#EC NOTEXT
  ls_rpy_dyfatc-text             = 'Ein.____________________________________'.                      "#EC NOTEXT
  ls_rpy_dyfatc-line             = '001'.                                                           "#EC NOTEXT
  ...
  ls_rpy_dyfatc-modific          = '4'.                                                             "#EC NOTEXT
  ls_rpy_dyfatc-requ_entry       = 'N'.                                                             "#EC NOTEXT
  ...
  ls_rpy_dyfatc-cont_name        = 'TCTRL_V_TN1TC_GLOB_SET'.                                        "#EC NOTEXT
  ls_rpy_dyfatc-type             = 'TEXT'.                                                          "#EC NOTEXT
  ls_rpy_dyfatc-name             = '*V_TN1TC_GLOB_SET-SECURITY_LEVEL'.                              "#EC NOTEXT
  ls_rpy_dyfatc-text             = 'S_______________________________________'.                      "#EC NOTEXT
  ls_rpy_dyfatc-line             = '001'.                                                           "#EC NOTEXT
  ...
  ls_rpy_dyfatc-cont_name        = 'TCTRL_V_TN1TC_GLOB_SET'.                                        "#EC NOTEXT
  ls_rpy_dyfatc-type             = 'TEXT'.                                                          "#EC NOTEXT
  ls_rpy_dyfatc-name             = '*V_TN1TC_GLOB_SET-DECAY_TIME'.                                  "#EC NOTEXT
  ls_rpy_dyfatc-text             = 'VS______________________________________'.                      "#EC NOTEXT
  ls_rpy_dyfatc-line             = '001'.                                                           "#EC NOTEXT
  ...
  ls_rpy_dyfatc-cont_name        = 'TCTRL_V_TN1TC_GLOB_SET'.                                        "#EC NOTEXT
  ls_rpy_dyfatc-type             = 'TEXT'.                                                          "#EC NOTEXT
  ls_rpy_dyfatc-name             = '*V_TN1TC_GLOB_SET-EINKB'.                                       "#EC NOTEXT
  ls_rpy_dyfatc-text             = 'Kurzname_der_Einr.______________________'.                      "#EC NOTEXT
  ls_rpy_dyfatc-line             = '001'.                                                           "#EC NOTEXT
  ...
  ls_rpy_dyfatc-cont_name        = 'TCTRL_V_TN1TC_GLOB_SET'.                                        "#EC NOTEXT
  ls_rpy_dyfatc-type             = 'TEXT'.                                                          "#EC NOTEXT
  ls_rpy_dyfatc-name             = '*V_TN1TC_GLOB_SET-NO_CLEAR_BUFFER'.                             "#EC NOTEXT
  ls_rpy_dyfatc-text             = 'NichtLeer_______________________________'.                      "#EC NOTEXT
  ls_rpy_dyfatc-line             = '001'.                                                           "#EC NOTEXT
  ls_rpy_dyfatc-column           = '005'.                                                           "#EC NOTEXT
  ls_rpy_dyfatc-length           = '040'.                                                           "#EC NOTEXT
  ls_rpy_dyfatc-vislength        = '009'.                                                           "#EC NOTEXT
  ls_rpy_dyfatc-height           = '001'.                                                           "#EC NOTEXT
  ls_rpy_dyfatc-format           = 'CHAR'.                                                          "#EC NOTEXT
  ls_rpy_dyfatc-from_dict        = 'X'.                                                             "#EC NOTEXT
  ...
  ls_rpy_dyfatc-conv_exit        = 'ALPHA'.                                                         "#EC NOTEXT
  ls_rpy_dyfatc-param_id         = 'EIN'.                                                           "#EC NOTEXT
  ...
  ls_rpy_dyfatc-vislength        = '003'.                                                           "#EC NOTEXT
  ls_rpy_dyfatc-height           = '001'.                                                           "#EC NOTEXT
  ls_rpy_dyfatc-format           = 'INT1'.                                                          "#EC NOTEXT
  ls_rpy_dyfatc-from_dict        = 'X'.                                                             "#EC NOTEXT
  ls_rpy_dyfatc-modific          = 'X'.                                                             "#EC NOTEXT
  ls_rpy_dyfatc-foreignkey       = 'X'.                                                             "#EC NOTEXT
  ls_rpy_dyfatc-input_fld        = 'X'.                                                             "#EC NOTEXT
  ...
  DATA: ls_rpy_dypara TYPE rpy_dypara, lt_rpy_dypara TYPE TABLE OF rpy_dypara.
    ...
    APPEND '   tctrl_v_tn1tc_glob_set CURSOR nextline.'                                   TO lt_d022s. "#EC NOTEXT
    ...
    APPEND '      FIELD v_tn1tc_glob_set-institution_id .'                                TO lt_d022s. "#EC NOTEXT
    APPEND '      FIELD v_tn1tc_glob_set-security_level .'                                TO lt_d022s. "#EC NOTEXT
    APPEND '      FIELD v_tn1tc_glob_set-decay_time .'                                    TO lt_d022s. "#EC NOTEXT
    APPEND '      FIELD v_tn1tc_glob_set-einkb .'                                         TO lt_d022s. "#EC NOTEXT
    APPEND '    FIELD V_TN1TC_GLOB_SET-NO_CLEAR_BUFFER .'                                 TO lt_d022s. "#EC NOTEXT
    APPEND '      MODULE set_update_flag ON CHAIN-REQUEST.'                               TO lt_d022s. "#EC NOTEXT
    APPEND '      MODULE complete_v_tn1tc_glob_set ON CHAIN-REQUEST.'                     TO lt_d022s. "#EC NOTEXT
...
FORM update_14. " TOBJ V_TN1TC_GLOB_SETV ( DE , source: EV8 )
  IF lcl_wb=>generate_maint_view(  "#EC *
      i_devclass     = 'N1BASE'
      i_objectname   = 'V_TN1TC_GLOB_SET'
      i_objecttype   = 'V'
      i_masterlang   = 'D'
      i_auth_group   = 'NC'
      i_func_group   = '0N1TC'
     ...
     PERFORM update_14_0100.
  ENDIF.
  CALL METHOD lcl_wb=>create_logical_object "#EC *
    EXPORTING
      i_object   = 'TOBJ'
      i_obj_name = 'V_TN1TC_GLOB_SETV'
      i_devclass = 'N1BASE'
      i_langu    = 'D'
    EXCEPTIONS
      OTHERS     = 1.
  ...
  CLEAR ls_objh.
  ls_objh-objectname     = 'V_TN1TC_GLOB_SET'.                                                      "#EC NOTEXT
  ...
  CLEAR ls_objs.
  ls_objs-objectname     = 'V_TN1TC_GLOB_SET'.                                                      "#EC NOTEXT
  ls_objs-objecttype     = 'V'.                                                                     "#EC NOTEXT
  ls_objs-tabname        = 'TN1TC_GLOB_SET'.                                                        "#EC NOTEXT
...
FORM update_15. " TABU TVIMF ( DE , source: EV8 )
  DATA: BEGIN OF ls_tvimf,
          tabname(30),
          event(2),
          formname(30),
        END OF ls_tvimf,
        lt_tvimf LIKE TABLE OF ls_tvimf.

  CLEAR ls_tvimf.
  ls_tvimf-tabname      = 'V_TN1TC_GLOB_SET'.                                                       "#EC NOTEXT
  ls_tvimf-event        = '01'.                                                                     "#EC NOTEXT
  ls_tvimf-formname     = 'Z01_SET_DATA_BEF_SAVE_100'.                                              "#EC NOTEXT
...
FORM genview.
  ...
  PERFORM update_14. " TOBJ V_TN1TC_GLOB_SETV ( DE , source: EV8 )
ENDFORM.
...
FORM update.
  ...
  APPEND 'TN1TC_GLOB_SET'                         TO gt_tabname.   "#EC NOTEXT
  APPEND 'V_TN1TC_GLOB_SET'                       TO gt_tabname.   "#EC NOTEXT

  PERFORM update_01. " FUGR 0N1TC ( DE , source: EV8 )
  PERFORM update_02. " DTED N1TC_NO_CLEAR_BUFFER ( DE , source: EV8 )
  PERFORM update_03. " DOCU DEN1TC_NO_CLEAR_BUFFER ( DE , source: EV8 )
  PERFORM update_04. " TABT N1TC_BUFFER ( DE , source: EV8 )
  PERFORM update_05. " INDX N1TC_BUFFER                   2 ( DE , source: EV8 )
  PERFORM update_06. " TABT N1TC_BUFFER_PAP ( DE , source: EV8 )
  PERFORM update_07. " TABT N1TC_MATRIX_BUF ( DE , source: EV8 )
  PERFORM update_08. " TABD TN1TC_GLOB_SET ( DE , source: EV8 )
  PERFORM update_09. " TABT TN1TC_GLOB_SET ( DE , source: EV8 )
  PERFORM update_10. " VIED V_TN1TC_GLOB_SET ( DE , source: EV8 )
  PERFORM update_11. " REPT SAPL0N1TC ( DE , source: EV8 )
  PERFORM update_12. " REPT RN1TC_CLEARBUFFER_BDC ( DE , source: EV8 )
  PERFORM update_13. " DOCU RERN1TC_CLEARBUFFER_BDC ( DE , source: EV8 )
  PERFORM update_14. " TOBJ V_TN1TC_GLOB_SETV ( DE , source: EV8 )
  PERFORM update_15. " TABU TVIMF ( DE , source: EV8 )
